...
const getLocationNameFromCoordinates = async (lat, lon) => {
  try {
    ...
    if (response.data.results && response.data.results.length > 0) {
      ...
      return (
        locality?.long_name || adminArea?.long_name || country?.long_name || "this area"
      );
    }
    return "this area";
  } catch (error) {
...
const getNearbyPlaces = async (lat, lon, radius = 20000) => {
  try {
    ...
    const places = response.data.results.map((place) => ({
      ...
      rating: place.rating || "N/A",
      vicinity: place.vicinity || null,
      photos: place.photos ? place.photos.map(photo => getPhotoUrl(photo.photo_reference)) : [],
    }));
    placesCache.set("attractions", lat, lon, radius, places);
    return places;
  } catch (error) {
...
const categorizeAttractions = (attractions) => {
  ...
  attractions.forEach((place) => {
    const types = place.types || [];
    if (types.includes("museum")) categories.museums.push(place);
    else if (types.includes("park") || types.includes("natural_feature"))
      categories.nature.push(place);
    else if (
      ...
      types.some((type) => type.includes("historical") || type.includes("landmark"))
    )
      categories.historical.push(place);
    else if (types.includes("neighborhood") || types.includes("sublocality"))
      categories.neighborhoods.push(place);
    else if (types.includes("point_of_interest")) categories.landmarks.push(place);
    else categories.other.push(place);
  });
  return categories;
};
...
const generateDiverseLocationSets = (
  attractions,
  userLat,
  userLon,
  numSets = 10,
  ...
  for (let i = 0; i < numSets && i < themeSets.length; i++) {
    ...
    const weights = availableForTheme.map(
      (place) => 1 / (place.distanceFromUser + 0.01)
    );
    const selectedLocations = getWeightedRandomSelection(
      availableForTheme,
      locationsPerSet,
...
const fetchGeminiExperiences = async (nearbyPlaces, latitude, longitude) => {
  try {
    const locationName = await getLocationNameFromCoordinates(latitude, longitude);
    const diverseLocationSets = generateDiverseLocationSets(
      nearbyPlaces,
      latitude,
      longitude
    );
    ...
    for (let i = 0; i < diverseLocationSets.length; i++) {
      const locationSet = diverseLocationSets[i];
      if (!locationSet.locations || locationSet.locations.length < 2) continue;
      const themePrompt = `You are a passionate and highly knowledgeable local tour guide from ${locationName} with decades of experience. Your mission is to craft a single immersive, one-of-a-kind tour experience with the theme "${locationSet.theme}". For EACH of the following REAL locations, provide a rich, detailed 300-500 word narration in first-person as if you're standing there with travelers: ${locationSet.locations.map(
        (loc) => `- ${loc.locationName}`
      ).join("\n")} Your narration for each location MUST include: - A warm welcome and orientation to the location - Vivid sensory descriptions (sights, sounds, smells, textures) - Fascinating historical context with specific dates, names, and events - Personal anecdotes and observations as a local guide - Cultural significance and how locals interact with this place - Hidden details most tourists miss - Practical tips (best photo spots, quieter areas, accessibility notes) - 2-3 specific questions you might ask visitors to engage them - Connections to other locations in the tour - A compelling transition to the next location Format your response as a JSON object, following this structure EXACTLY: { "title": "An engaging title for your '${locationSet.theme}' themed tour", "description": "Overall tour description connecting these specific locations", "locations": [ { "locationName": "${locationSet.locations[0].locationName}", "lat": ${locationSet.locations[0].lat}, "lon": ${locationSet.locations[0].lon}, "narration": "Your detailed 300-500 word narration here..." }, ... (continue for all provided locations) ] } Return ONLY the JSON object with no additional text or explanations. Important: Do not start every tour description with the phrase "Embark on...". Use a variety of engaging opening phrases and sentence structures to capture the essence of each tour. Vary the tone of each description, using a mix of formal, informal, enthusiastic, and reflective language.`;
      try {
        const result = await model.generateContent({
          contents: [{ role: "user", parts: [{ text: themePrompt }] }],
        });
        ...
        const experience = JSON.parse(cleanedResponse);
        const enhancedLocations = experience.locations.map((location) => {
          const matchingPlace = locationSet.locations.find(
            (place) =>
              place.locationName.toLowerCase() === location.locationName.toLowerCase() ||
              place.locationName.toLowerCase().includes(location.locationName.toLowerCase()) ||
              location.locationName.toLowerCase().includes(place.locationName.toLowerCase())
        ...
        console.error(
          `Error generating experience for theme ${locationSet.theme}:`,
          error
        );
      }
    }
...
const rateLimiter = {
  ...
  check: function (ip) {
    ...
    if (request.count >= this.limit) return false;
    request.count++;
    return true;
  },
};
  ...
  experiences.forEach((exp) => {
    ...
    exp.locations.forEach((loc) => {
      const distance = Math.sqrt(
        Math.pow(loc.lat - userLat, 2) + Math.pow(loc.lon - userLon, 2)
      );
      proximityScore += Math.max(0, 10 - distance * 500);
    });
    proximityScore = proximityScore / exp.locations.length;
...
const ensureUniqueDescriptions = (experiences) => {
  const repetitivePhrases = ["Embark on...", "Discover...", "Experience..."];
  const replacementPhrases = {
    "Embark on...": ["Begin your journey...", "Set forth on...", "Delve into..."],
    "Discover...": ["Uncover...", "Explore...", "Find..."],
    "Experience...": ["Immerse yourself in...", "Witness...", "Partake in..."],
  };
  experiences.forEach((exp) => {
    exp.locations.forEach((loc) => {
      repetitivePhrases.forEach((phrase) => {
        if (loc.narration.startsWith(phrase)) {
          const replacements = replacementPhrases[phrase];
          const randomIndex = Math.floor(Math.random() * replacements.length);
          loc.narration = loc.narration.replace(phrase, replacements[randomIndex]);
        }
      });
    });
  });
...
router.post("/", rateLimit, async (req, res) => {
  try {
    ...
    if (!lat || !lon || !user_id)
      return res
        .status(400)
        .json({ error: "Latitude, longitude, and user_id are required." });
    const latitude = parseFloat(lat);
    const longitude = parseFloat(lon);
    const userPrefs = preferences ? JSON.parse(preferences) : {};
    const isUrbanArea = await isUrbanLocation(latitude, longitude);
    ...
    if (nearbyPlaces.length < 5)
      nearbyPlaces.push(...(await getNearbyPlaces(latitude, longitude, 10000)));
    const additionalAttractions = await searchForAdditionalAttractions(
      latitude,
      longitude
    );
    nearbyPlaces = [...nearbyPlaces, ...additionalAttractions];
    ...
    if (nearbyPlaces.length === 0)
      return res.status(404).json({ error: "No points of interest found" });
    let generatedExperiences = await fetchGeminiExperiences(
      nearbyPlaces,
      latitude,
      longitude
    );
    ...
    generatedExperiences = ensureUniqueDescriptions(generatedExperiences);
