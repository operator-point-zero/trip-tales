...
// Enhanced to fetch and categorize places with photos
const getNearbyPlaces = async (lat, lon, radius = 20000) => {
  try {
    // Check cache first
    const cacheKey = placesCache.getKey(lat, lon, radius);
    ...
    const places = await Promise.all(response.data.results.map(async place => {
        ...
        if (place.photos && place.photos.length > 0) {
            photos = await Promise.all(place.photos.slice(0, 2).map(async photo => {
                try {
                    ...
                    console.error("Failed to fetch photo:", photoError.message);
                    return null;
                }
            }));
            photos = photos.filter(photo => photo !== null); // Remove failed photo fetches
        }
        return {
            locationName: place.name,
            lat: place.geometry.location.lat,
...
// Function to search for additional types of attractions with photos
const searchForAdditionalAttractions = async (lat, lon) => {
  try {
    // Check cache first
    const cacheKey = placesCache.getKey(lat, lon, 15000);
    ...
    const attractions = await Promise.all(response.data.results.map(async place => {
        let photos = [];
        if (place.photos && place.photos.length > 0) {
            photos = await Promise.all(place.photos.slice(0, 2).map(async photo => {
                try {
...
// Modified to generate diverse experiences with photos
const fetchGeminiExperiences = async (nearbyPlaces, latitude, longitude) => {
  try {
    // Get location name for the prompt
    const locationName = await getLocationNameFromCoordinates(latitude, longitude);
  ...
  try {
    ...
    const newExperiences = generatedExperiences.map(exp => ({
      ...
      locations: exp.locations.map(loc => ({
        ...
        rating: loc.rating || null,
        vicinity: loc.vicinity || null,
        photos: loc.photos || [], // Include photos
        narration: loc.narration || `Welcome to ${loc.locationName}, one of the must-visit spots in our tour.`
      })),
      user_id,
      is_seed: true,
